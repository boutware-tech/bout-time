...
/**
 * A utility class to make bouts for a 31-man group in a 32-man bracket.
 * Top 1 seed has a first round bye.
 * @deprecated Replaced with bouttime.boutmaker.bracket32.Bracket32BoutMaker
 */
public class Bracket31BoutMaker extends Bracket32BoutMaker {

    protected List<Bout> makeRound1Bouts(List<Wrestler> wList, Group g, Wrestler d) {
        List<Bout> bList = super.makeRound1Bouts(g, d);
        bList.remove(r1b1);
        return bList;
    }
    
    protected List<Bout> makeRound2Bouts(List<Wrestler> wList, Group g, Wrestler d) {
        List<Bout> bList = super.makeRound2Bouts(g, d);
        r2b1.setRed(wList.get(0));
        bList.remove(r2b9);
        return bList;
    }
}
